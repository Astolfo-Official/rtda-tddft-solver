...
def gen_tda_operation(mf, fock_ao=None, singlet=True, wfnsym=None):
    ...
    def vind(zs, trans=False):
        if trans :
            zs = numpy.asarray(zs).reshape(nocc,nvir,-1)
            zs = zs.transpose(1,2,0)
        ...
        if wfnsym is not None and mol.symmetry:
            v1ov[:,sym_forbid] = 0
        #test = lib.einsum('xjb,iajb->xia',zs,aeo)
        #test -=v1ov
        #print(numpy.mean(abs(test)))
        v1ov = v1ov.reshape(v1ov.shape[0],-1)
        if trans :
            v1ov = v1ov.transpose(1,0)
        return v1ov
...
class TDA(TDMixin):
    ...
    '''
    def gen_vind(self, mf=None, dm0=None, mo_coeff0=None, mo_occ0=None):
    ...
    def kernel(self, x0=None, nstates=None):
        ...
        from pydmet.solvers.afdavidson import af_davidson as aftda
        self.e, alpha_v = aftda(mf, vind, hdiag, nstate=self.nstates, conv_tol=1e-5, max_cycle=self.max_cycle, verbose=log)
        self.xy = []
        mo_occ = mf.mo_occ
        occidx = numpy.where(mo_occ==2)[0]
        viridx = numpy.where(mo_occ==0)[0]
        nocc = len(occidx)
        nvir = len(viridx)
        for i in range(numpy.shape(alpha_v)[0]):
            self.xy.append((alpha_v[i].reshape(nocc,nvir)*numpy.sqrt(.5),0))
        '''
        if x0 is None:
